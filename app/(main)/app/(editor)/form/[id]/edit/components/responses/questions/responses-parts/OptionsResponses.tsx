...
export default function OptionsResponses({
  sq,
  isQuiz,
}: {
  ...
  const answers = sq?.map((i, idx) => {
    let isCorrect = false;
    if (i.correctAnswers?.length && i.correctAnswers?.length === i.answers.length) {
      const sortedCorrectAnswers = i.correctAnswers.sort();
      const sortedAnswers = i.answers.sort();
      if (sortedCorrectAnswers.every((ca, idx) => ca === sortedAnswers[idx]))
        isCorrect = true;
    }
    return {
      id: `${i.id}_${idx}`,
      options: i.answers
        .map((a) => ` ${i.options.find((o) => o.id === a)?.title}`)
        .toString(),
      isCorrect,
    };
  });
